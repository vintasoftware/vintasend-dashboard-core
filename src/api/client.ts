...
export type VintaSendClientConfig = {
  /**
...
export type VintaSendFetchClient = ReturnType<typeof createFetchClient<paths>>;

function authMiddleware(config: VintaSendClientConfig): Middleware {
  return {
...
/**
 * Builds the typed fetch client. Most apps want `createVintaSendClient`
 * instead, which also wires up the TanStack Query hooks.
 */
export function createVintaSendFetchClient(config: VintaSendClientConfig): VintaSendFetchClient {
