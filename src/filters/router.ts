...
/**
 * How the filter hook talks to a router.
 *
 * The point of this package is that the UI and its routing are yours, so the
 * hook never imports a router. It reads and writes search params through this
 * two-method interface; `useHistoryRouterAdapter` below implements it on the
 * History API and works in any React app, and
 * `vintasend-dashboard-core/next` ships one for the Next.js app router.
 */
 ...
 */
const SEARCH_PARAMS_EVENT = 'vintasend:searchparams';
