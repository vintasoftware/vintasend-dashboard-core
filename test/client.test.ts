...
import { createVintaSendFetchClient } from '../src/api/client.js';
...
describe('createVintaSendFetchClient', () => {
  it('sends the request to the configured base URL', async () => {
    const { calls, fetchImpl } = recordingFetch();

    const client = createVintaSendFetchClient({
    ...
    expect(lastRequest(calls).url).toBe('https://api.example.com/api/v1/notifications');
  });
